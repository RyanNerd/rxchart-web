 ...
 */
export const getCheckoutList = (drugLogList: DrugLogRecord[]): DrugLogRecord[] => {
    return multiSort(
        drugLogList.filter((drug) => {
            return drug.Out && drug.Out > 0 && (drug.In === null || drug.In === 0) && isThisDay(drug);
        }),
